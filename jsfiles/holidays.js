...
fetchFirebaseConfigJson()
  ...
  .then(async data => {
    ...
    const holidates = (admission[1] && admission[1].holidates) || [];

    const ulElement = document.getElementById("hld_ul");
    if (ulElement) {
      holidates.forEach((feature) => {
    ...
    const hldtitleElement = document.getElementById("hldTitle");
    if (hldtitleElement && fetch.holidaysystemTitle) {
      hldtitleElement.innerHTML = fetch.holidaysystemTitle.replace(/\n/g, '<br/>');
    }

    const hldtextElement = document.getElementById("hldText");
    if (hldtextElement) {
      hldtextElement.innerHTML = (fetch.holidaysystemText || "").replace(/\n/g, '<br/>');
    }
